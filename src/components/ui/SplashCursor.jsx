...
/**
 * SplashCursor - ReactBits Cosmic Fluid Cursor
 * 
 * WebGL-based fluid simulation cursor effect customized for cosmic theme.
 * Colors: Neon Cyan (#00f2ff), Deep Purple (#7000ff), White
 * 
 * Props from ReactBits with cosmic defaults:
 * - BACK_COLOR: transparent (Galaxy visible)
 * - DYE_RESOLUTION: 512 (high quality)
 * - SPLAT_RADIUS: 0.2 (star-like splash)
 * - SHADING: true (depth against stars)
 */
function SplashCursor({
  ...
  useEffect(() => {
    ...
    const gradienSubtractShader = compileShader(
      gl.FRAGMENT_SHADER,
      `
        precision mediump float;
        precision mediump sampler2D;
    ...
    // Cosmic color palette: Neon Cyan, Deep Purple, White
    const cosmicColors = [
      { r: 0, g: 0.95, b: 1 },      // Neon Cyan #00f2ff
      { r: 0.44, g: 0, b: 1 },      // Deep Purple #7000ff
      { r: 1, g: 1, b: 1 },         // White
      { r: 0, g: 0.8, b: 0.9 },     // Light Cyan
      { r: 0.6, g: 0.2, b: 1 },     // Light Purple
    ];
    ...
    canvas.addEventListener('touchstart', e => {
      ...
      for (let i = 0; i < touches.length; i++) {
        if (pointers[i] == null) pointers.push(new pointerPrototype());
    ...
    canvas.addEventListener('touchmove', e => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        if (pointer) updatePointerMoveData(pointer, touches[i].pageX, touches[i].pageY);
      }
    }, { passive: false });
